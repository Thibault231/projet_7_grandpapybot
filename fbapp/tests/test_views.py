...
def test_true_api_all_post():
    ...
    assert isinstance(file['summary'], str)\
        ...
        and file['status-code'] == '200-Response complete'\
...
def test_empty_api_all_post():
    ...
    assert file['success'] is False\
        and file['status-code'] == 'Error-400-no question'
...
def test_nokeyword_api_all_post():
    ...
    file = response.json()
    assert file['success'] is False\
        and file['status-code'] == 'Error-400-no keyword in the question'
...
def test_noplace_api_all_post():
    ...
    file = response.json()
    assert file['success'] is False\
        and file['status-code'] == 'Error-404-place not found'
